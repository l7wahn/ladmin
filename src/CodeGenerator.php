...
class CodeGenerator
{
	...
	**/
	public static function createController($config, $comm = null) {
        ...
        $path = base_path('app/Http/Controllers/LA/'.$config->controllerName.".php");
        $baseStub = $config->isUserChild ? file_get_contents($templateDirectory."/controller_base.stub") : file_get_contents($templateDirectory."/userchildcontroller.stub");

        if (!is_dir(base_path('app/LA/Autogenerated/Controllers'))) {            
            self::forceDir(base_path('app/LA/Autogenerated/Controllers'));
        }

        $basePath = base_path('app/LA/Autogenerated/Controllers/'.$config->controllerName.".php");
        if(!file_exists ($path))
        {
        ...
        $contents = $baseStub;
        $contents = str_replace("__controller_class_name__", $config->controllerName, $contents);
        if (preg_match('\'<la>(.*?)</la>\'si', $contents, $match) == 1) {
            $autofields = file_get_contents($templateDirectory."/controller_autofields.stub");
            $autofields = str_replace("__module_name__", $config->moduleName, $autofields);
            $md  = preg_replace('\'<la>(.*?)</la>\'si', "<la> AutoGenerated DO NOT EDIT!! \n\n ". $autofields . " \n\n // END </la>", $contents);
            self::forceFilePutContents($basePath, $md);
        }                        
    }

    private static function forceDir($dir){
        if(!is_dir($dir)){
            $dir_p = explode('/',$dir);
    ...
    public static function createModel($config, $comm = null) {
        ...
        $templateDirectory = __DIR__.'/stubs';
        $path = base_path('app/Models/'.$config->modelName.".php");        
        $baseStub = file_get_contents($templateDirectory."/model_base.stub");
        if (!is_dir(base_path('app/LA/Autogenerated/Models'))) {            
            self::forceDir(base_path('app/LA/Autogenerated/Models'));
        }
        $basePath = base_path('app/LA/Autogenerated/Models/'.$config->modelName.".php");
        if(!file_exists ($path))
        {    
        ...
        $contents = $baseStub;
        $contents = str_replace("__model_class_name__", $config->modelName, $contents);
        if (preg_match('\'<la>(.*?)</la>\'si', $contents, $match) == 1) {
            $autofields = file_get_contents($templateDirectory."/model_autofields.stub");
            $autofields = str_replace("__db_table_name__", $config->dbTableName, $autofields);
            $md  = preg_replace('\'<la>(.*?)</la>\'si', "<la> AutoGenerated DO NOT EDIT!! \n\n ". $autofields . " \n\n // END </la>", $contents);
            self::forceFilePutContents($basePath, $md);
        }  
	...
	**/
    ...
    public static function generateConfig($module, $icon, $is_user_child = false)
    {
        $config = array();
        ...
        $config->isUserChild = $is_user_child;
